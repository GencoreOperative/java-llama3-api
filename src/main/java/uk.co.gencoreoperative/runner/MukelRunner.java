...
public class MukelRunner implements Run {
     ...
     */
    public @Nonnull String run(@Nonnull Config options) {
        ...
        RunAsync.runAsync(getRunnable(args, output, error));
     ...
     */
    public @Nonnull Stream<String> runAsStream(@Nonnull Config options) {
        BlockingQueueIterator output = new BlockingQueueIterator();
        BlockingQueueIterator error = new BlockingQueueIterator();
        String[] args = buildArgs(options);
        RunAsync.runAsync(getRunnable(args, output, error));
     ...
     */
    @Override
    public @Nonnull Response runWithResponse(@Nonnull Config options) throws RuntimeException {
        ...
        getRunnable(args, output, error).run();
    ...
    private Runnable getRunnable(String[] args, BlockingQueueIterator output, BlockingQueueIterator error) {
        if (modelType == LLAMA3) {
            return () -> {
